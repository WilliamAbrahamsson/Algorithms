...
public class Graph {
  ...
  // has edge func.
  public boolean has_edge(int from, int to) {
    Iterator<Edge> iterate_adjacencies = iterate_adjacencies(from);
  ...
  public double get_weight(int from, int to) {
    ArrayList<Edge> adjList = adjacency_list.get(from);
    for (int i = 0; i < adjList.size(); i++) {
      if (adjList.get(i).to == to) {
        return adjList.get(i).getWeight();
      }
    }
    // Return -1 if no such edge exists.
    return -1;
  }
